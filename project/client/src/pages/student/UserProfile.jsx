...
import { User, Mail, Hash, Building, BookOpen, Phone, FileText, Edit3, Save, X, Award } from 'lucide-react';
...
export default function UserProfile() {
    ...
    const Field = ({ icon: Icon, label, value }) => (
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '14px 0', borderBottom: '1px solid var(--border)' }}>
            <div style={{ width: 34, height: 34, borderRadius: 9, background: 'rgba(22,163,74,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 24 }}>
                ...
                <div className="glass" style={{ padding: 24, textAlign: 'center', alignSelf: 'start' }}>
                    <div style={{ position: 'relative', width: 80, height: 80, margin: '0 auto 16px' }}>
                        {user?.avatar ? (
                            <img src={user.avatar.startsWith('http') ? user.avatar : (user.avatar.startsWith('/uploads') ? `http://localhost:5000${user.avatar}` : user.avatar)} alt="Profile" style={{ width: 80, height: 80, borderRadius: '50%', objectFit: 'cover', border: '2px solid var(--primary)' }} />
                        ) : (
                            <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'linear-gradient(135deg, var(--primary), var(--secondary))', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, fontWeight: 800, color: 'white' }}>
                                {user?.name?.[0]?.toUpperCase()}
                            </div>
                        )}
                        {editing && (
                            <label style={{ position: 'absolute', bottom: -5, right: -5, width: 28, height: 28, background: 'var(--primary)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', border: '2px solid white', color: 'white' }}>
                                <Edit3 size={14} />
                                <input type="file" hidden accept="image/*" onChange={e => setAvatarFile(e.target.files[0])} />
                            </label>
                        )}
                    </div>
                    {avatarFile && <div style={{ fontSize: 10, color: 'var(--primary)', marginBottom: 10 }}>New photo selected: {avatarFile.name}</div>}
                    <div style={{ fontWeight: 800, fontSize: 16, color: 'var(--text)' }}>{user?.name}</div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>{user?.role}</div>
                    <div style={{ marginTop: 14, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, background: 'rgba(245,158,11,0.08)', padding: '8px 14px', borderRadius: 20, border: '1px solid rgba(245,158,11,0.2)' }}>
                        <Award size={14} color="#f59e0b" />
                        <span style={{ fontSize: 13, fontWeight: 700, color: '#d97706' }}>{user?.helpfulnessScore || 0} pts</span>
                    </div>
                </div>
                    ...
                    {!editing ? (
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginTop: 12 }}>
                            {[
                                ...
                                { key: 'batch', label: 'Batch Year', type: 'number' },
                            ...
                            <div>
                                <label style={{ display: 'block', fontSize: 12, color: 'var(--text-muted)', marginBottom: 6, fontWeight: 500 }}>Bio</label>
                                <textarea className="input" rows={3} value={form.bio} onChange={e => set('bio', e.target.value)} placeholder="Tell something about yourself..." style={{ resize: 'vertical' }} />
                            </div>
                        </div>
                    )}
                </div>
